...
#Create dates and split it into month and year
...
Is the effect of increasing temperature more pronounced in some months? We use `facet_wrap()` to produce a separate scatter plot for each month, again with a smoothing line. Our chart now has human-readable labels; that is, each month is labeled "Jan", "Feb", "Mar" (full or abbreviated month names are fine), not `1`, `2`, `3`. 
...
ggplot(tidyweather,aes(x=Year,y=delta,color=month))+
  ...
  facet_wrap(~month,
             scales = "free")+
  theme(plot.title = element_text(hjust = 0.5,size=10,margin=margin(0,0,10,0)),strip.text = element_text(size=12))+
...
#Calculating confidence interval by applying formula manually
formula_ci <- comparison %>% 
  filter(interval=="2011-present") %>% #We only focus on this period
...
bootstrap_delta<- comparison %>% 
  ...
  calculate(stat = "mean")
#Looking at 1000 times simulation results
bootstrap_delta
#Retracting confidence interval according to our simulation distribution
...
#Giving initials to every month
month_labs<-c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
...
weekly_avg_new
#calculate weekly averages for every week of these 6 years seperately
